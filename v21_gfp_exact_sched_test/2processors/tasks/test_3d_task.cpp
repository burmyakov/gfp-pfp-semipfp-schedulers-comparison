        ...
        if (itr2->first > s.p[2]) break;
            ...
            for (mt1::reverse_iterator itr1 = (itr2->second).rbegin(); itr1 != (itr2->second).rend(); itr1++) {
                if (itr1->first < s.c[2]) break;
                    ...
                    cout << "c[3]: " << itr1->first << endl;
                    cout << "p[3]: " << itr2->first << endl;*/
                    return true;
                }
            }
        }
...
bool find_dominating_state_hpt2(const state& s, const TS& ts, mt4& visitedStates) {
    
    if (s.p[1] <= ts.P[1] - ts.C[1]) { // s.c[1] == 0
        for (mt4::iterator itr4 = visitedStates.begin(); itr4 != visitedStates.end(); itr4++) {
            if (itr4->first > s.p[1]) break;
            else {
                if (find_dominating_state_lpts(s, ts, itr4->second)) {
                    //cout << "p[2]: " << (int)itr4->first << endl;
                    return true;
                }
            }
        }
    } else { // s.c[1] > 0
        if (find_dominating_state_lpts(s, ts, visitedStates[s.p[1]])) {
            //cout << "p[2]: " << (int)s.p[1] << endl;
            return true;
        }
    }
...
bool find_dominating_state_hpt1(const state& s, const TS& ts, mt6& visitedStates) {
    ...
    if (s.p[0] <= ts.P[0] - ts.C[0]) { // s.c[0] == 0
        for (mt6::iterator itr6 = visitedStates.begin(); itr6 != visitedStates.end(); itr6++) {
            if (itr6->first > s.p[0]) break;
                ...
                if (find_dominating_state_hpt2(s, ts, itr6->second)) {
                    //cout << "p[1]: " << (int)itr6->first << endl;
                    return true;
                }
            }
        }
    } else { // s.c[0] > 0
        if (find_dominating_state_hpt2(s, ts, visitedStates[s.p[0]])) {
            //cout << "p[1]: " << (int)s.p[0] << endl;
            return true;
        }
    }
...
bool find_dominating_state(const state& s, const unsigned short m, const TS& ts, mt7* visitedStates) {
    
    unsigned int keysNum_pj = 0;
    get_keys_pj(s, m, keysNum_pj, bKeys_pj);
    
    for (int i7 = 0; i7 < keysNum_pj; i7++) {
        if ((*visitedStates).find((*bKeys_pj)[i7]) != (*visitedStates).end()) {
            if (find_dominating_state_hpt1(s, ts, (*visitedStates)[(*bKeys_pj)[i7]])) {
                //cout << "map key: " << (*bKeys_pj)[i7] << endl;
                return true;
            }
        }
    }
...
void add_state_to_map(const state& s, const TS& ts, mt7* visitedStates) {
    
    my_bitset bs_pj;
    for (int i = 0; i < ts.n; i++) if (s.c[i] > 0) bs_pj.set(i, 1); else bs_pj.set(i, 0);
    
    (*visitedStates)[bs_pj][s.p[0]][s.p[1]][s.p[2]][s.c[2]] = true;
}
...
bool update_map(const state& s, const unsigned short m, const TS& ts, mt7* visitedStates) {
...
bool analyse_state(state& s, const TS& ts, const unsigned short m, priority_queue<state>& q, mt7* generated, const bool verbose) {
...
bool populate(const unsigned short m, const TS& ts_, const bool verbose) {
    ...
    if (verbose) {
        ...
        cout << "Visited states with tau_3 pending: " << visitedStatesNum_tauKpending << "  ( " << (float)((int)(visitedStatesNum_tauKpending*10000/visitedStatesNum))/100 << " %)" << endl << endl;
    }
...
bool test_3d_task(const bool verbose_, const unsigned short m_, const TS& ts_, unsigned long int& savedStatesNum_, unsigned long int& visitedStatesNum_) {
    
    bool schedulable = NS_3tasks::populate(m_, ts_, verbose_);
