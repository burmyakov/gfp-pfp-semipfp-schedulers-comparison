...
#include "../custom_types/map_13_tasks.h"
...
bool find_dominating_state_lpts(const state& s, const TS& ts, mt22& visitedStates) {
...
bool find_dominating_state_hpt2(const state& s, const TS& ts, mt24& visitedStates) {
    
    if (s.p[1] <= ts.P[1] - ts.C[1]) { // s.c[1] == 0
        for (mt24::iterator itr24 = visitedStates.begin(); itr24 != visitedStates.end(); itr24++) {
            if (itr24->first > s.p[1]) break;
            else {
                if (find_dominating_state_lpts(s, ts, itr24->second)) return true;
            }
        }
    } else { // s.c[1] > 0
...
bool find_dominating_state_hpt1(const state& s, const TS& ts, mt26& visitedStates) {
    
    if (s.p[0] <= ts.P[0] - ts.C[0]) { // s.c[0] == 0
        for (mt26::iterator itr26 = visitedStates.begin(); itr26 != visitedStates.end(); itr26++) {
            if (itr26->first > s.p[0]) break;
            else {
                if (find_dominating_state_hpt2(s, ts, itr26->second)) return true;
            }
        }
    } else { // s.c[0] > 0
...
bool find_dominating_state(const state& s, const unsigned short m, const TS& ts, mt27* visitedStates) {
    
    unsigned int keysNum_pj = 0;
    get_keys_pj(s, m, keysNum_pj, bKeys_pj);
    
    for (unsigned int i27 = 0; i27 < keysNum_pj; i27++) {
        if ((*visitedStates).find((*bKeys_pj)[i27]) != (*visitedStates).end()) {
            if (find_dominating_state_hpt1(s, ts, (*visitedStates)[(*bKeys_pj)[i27]])) return true;
        }
    }
...
void add_state_to_map(const state& s, const TS& ts, mt27* visitedStates) {
    
    my_bitset bs_pj;
    for (int i = 0; i < ts.n; i++) if (s.c[i] > 0) bs_pj.set(i, 1); else bs_pj.set(i, 0);
    
    (*visitedStates)[bs_pj][s.p[0]][s.p[1]][s.p[12]][s.c[12]][s.p[11]][s.c[11]][s.p[10]][s.c[10]][s.p[9]][s.c[9]][s.p[8]][s.c[8]][s.p[7]][s.c[7]][s.p[6]][s.c[6]][s.p[5]][s.c[5]][s.p[4]][s.c[4]][s.p[3]][s.c[3]][s.p[2]][s.c[2]] = true;
}
...
bool update_map(const state& s, const unsigned short m, const TS& ts, mt27* visitedStates) {
...
bool analyse_state(state& s, const TS& ts, const unsigned short m, priority_queue<state>& q, mt27* generated, const bool verbose) {
...
bool populate(const unsigned short m, const TS& ts_, const bool verbose) {
    ...
    if (verbose) {
        ...
        if (savedStatesNum > 0) cout << "Saved states with tau_13 pending: " << savedStatesNum_tauKpending << "  ( " << (float)((int)(savedStatesNum_tauKpending*10000/savedStatesNum))/100 << " %)" << endl;
        cout << "Visited states:\t\t" << visitedStatesNum << endl;
        cout << "Visited states with tau_13 pending: " << visitedStatesNum_tauKpending << "  ( " << (float)((int)(visitedStatesNum_tauKpending*10000/visitedStatesNum))/100 << " %)" << endl << endl;
    }
    
    return schedulable;
}
} // end of namespace NS_13tasks





bool test_13th_task(const bool verbose_, const unsigned short m_, const TS& ts_, unsigned long int& savedStatesNum_, unsigned long int& visitedStatesNum_) {
    
    bool schedulable = NS_13tasks::populate(m_, ts_, verbose_);
    savedStatesNum_ = NS_13tasks::savedStatesNum;
    visitedStatesNum_ = NS_13tasks::visitedStatesNum;
