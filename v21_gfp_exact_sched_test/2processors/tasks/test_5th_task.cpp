...
bool find_dominating_state_lpts(const state& s, const TS& ts, mt6& visitedStates) {
...
bool find_dominating_state_hpt2(const state& s, const TS& ts, mt8& visitedStates) {
    ...
    if (s.p[1] <= ts.P[1] - ts.C[1]) { // s.c[1] == 0
        for (mt8::iterator itr8 = visitedStates.begin(); itr8 != visitedStates.end(); itr8++) {
            if (itr8->first > s.p[1]) break;
                ...
                if (find_dominating_state_lpts(s, ts, itr8->second)) return true;
            }
        }
    } else { // s.c[1] > 0
...
bool find_dominating_state_hpt1(const state& s, const TS& ts, mt10& visitedStates) {
    ...
    if (s.p[0] <= ts.P[0] - ts.C[0]) { // s.c[0] == 0
        for (mt10::iterator itr10 = visitedStates.begin(); itr10 != visitedStates.end(); itr10++) {
            if (itr10->first > s.p[0]) break;
                ...
                if (find_dominating_state_hpt2(s, ts, itr10->second)) return true;
            }
        }
    } else { // s.c[0] > 0
...
bool find_dominating_state(const state& s, const unsigned short m, const TS& ts, mt11* visitedStates) {
    
    unsigned int keysNum_pj = 0;
    get_keys_pj(s, m, keysNum_pj, bKeys_pj);
    
    for (unsigned int i11 = 0; i11 < keysNum_pj; i11++) {
        if ((*visitedStates).find((*bKeys_pj)[i11]) != (*visitedStates).end()) {
            if (find_dominating_state_hpt1(s, ts, (*visitedStates)[(*bKeys_pj)[i11]])) return true;
        }
    }
...
void add_state_to_map(const state& s, const TS& ts, mt11* visitedStates) {
    
    my_bitset bs_pj;
    for (int i = 0; i < ts.n; i++) if (s.c[i] > 0) bs_pj.set(i, 1); else bs_pj.set(i, 0);
    
    (*visitedStates)[bs_pj][s.p[0]][s.p[1]][s.p[4]][s.c[4]][s.p[3]][s.c[3]][s.p[2]][s.c[2]] = true;
}
...
bool update_map(const state& s, const unsigned short m, const TS& ts, mt11* visitedStates) {
...
bool analyse_state(state& s, const TS& ts, const unsigned short m, priority_queue<state>& q, mt11* generated, const bool verbose) {
...
bool populate(const unsigned short m, const TS& ts_, const bool verbose) {
    ...
    if (verbose) {
        ...
        cout << "Visited states with tau_5 pending: " << visitedStatesNum_tauKpending << "  ( " << (float)((int)(visitedStatesNum_tauKpending*10000/visitedStatesNum))/100 << " %)" << endl << endl;
    }
...
bool test_5th_task(const bool verbose_, const unsigned short m_, const TS& ts_, unsigned long int& savedStatesNum_, unsigned long int& visitedStatesNum_) {
    
    bool schedulable = NS_5tasks::populate(m_, ts_, verbose_);
