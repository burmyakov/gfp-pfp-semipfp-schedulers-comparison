...
#include "../custom_types/map_8_tasks.h"
...
bool find_dominating_state_lpts(const state& s, const TS& ts, mt12& visitedStates) {
...
bool find_dominating_state_hpt2(const state& s, const TS& ts, mt14& visitedStates) {
    
    if (s.p[1] <= ts.P[1] - ts.C[1]) { // s.c[1] == 0
        for (mt14::iterator itr14 = visitedStates.begin(); itr14 != visitedStates.end(); itr14++) {
            if (itr14->first > s.p[1]) break;
            else {
                if (find_dominating_state_lpts(s, ts, itr14->second)) return true;
            }
        }
    } else { // s.c[1] > 0
...
bool find_dominating_state_hpt1(const state& s, const TS& ts, mt16& visitedStates) {
    ...
    if (s.p[0] <= ts.P[0] - ts.C[0]) { // s.c[0] == 0
        for (mt16::iterator itr16 = visitedStates.begin(); itr16 != visitedStates.end(); itr16++) {
            if (itr16->first > s.p[0]) break;
                ...
                if (find_dominating_state_hpt2(s, ts, itr16->second)) return true;
            }
        }
    } else { // s.c[0] > 0
...
bool find_dominating_state(const state& s, const unsigned short m, const TS& ts, mt17* visitedStates) {
    
    unsigned int keysNum_pj = 0;
    get_keys_pj(s, m, keysNum_pj, bKeys_pj);
    
    for (unsigned int i17 = 0; i17 < keysNum_pj; i17++) {
        if ((*visitedStates).find((*bKeys_pj)[i17]) != (*visitedStates).end()) {
            if (find_dominating_state_hpt1(s, ts, (*visitedStates)[(*bKeys_pj)[i17]])) return true;
        }
    }
...
void add_state_to_map(const state& s, const TS& ts, mt17* visitedStates) {
    
    my_bitset bs_pj;
    for (int i = 0; i < ts.n; i++) if (s.c[i] > 0) bs_pj.set(i, 1); else bs_pj.set(i, 0);
    
    (*visitedStates)[bs_pj][s.p[0]][s.p[1]][s.p[7]][s.c[7]][s.p[6]][s.c[6]][s.p[5]][s.c[5]][s.p[4]][s.c[4]][s.p[3]][s.c[3]][s.p[2]][s.c[2]] = true;
}
...
bool update_map(const state& s, const unsigned short m, const TS& ts, mt17* visitedStates) {
...
bool analyse_state(state& s, const TS& ts, const unsigned short m, priority_queue<state>& q, mt17* generated, const bool verbose) {
...
bool populate(const unsigned short m, const TS& ts_, const bool verbose) {
    ...
    if (verbose) {
        ...
        if (savedStatesNum > 0) cout << "Saved states with tau_8 pending: " << savedStatesNum_tauKpending << "  ( " << (float)((int)(savedStatesNum_tauKpending*10000/savedStatesNum))/100 << " %)" << endl;
        cout << "Visited states:\t\t" << visitedStatesNum << endl;
        cout << "Visited states with tau_8 pending: " << visitedStatesNum_tauKpending << "  ( " << (float)((int)(visitedStatesNum_tauKpending*10000/visitedStatesNum))/100 << " %)" << endl << endl;
    }
    
    return schedulable;
}
} // end of namespace NS_8tasks





bool test_8th_task(const bool verbose_, const unsigned short m_, const TS& ts_, unsigned long int& savedStatesNum_, unsigned long int& visitedStatesNum_) {
    
    bool schedulable = NS_8tasks::populate(m_, ts_, verbose_);
    savedStatesNum_ = NS_8tasks::savedStatesNum;
    visitedStatesNum_ = NS_8tasks::visitedStatesNum;
