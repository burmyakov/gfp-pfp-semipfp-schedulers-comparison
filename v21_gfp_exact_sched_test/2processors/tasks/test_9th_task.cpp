...
bool find_dominating_state(const state& s, const unsigned short m, const TS& ts, mt19* visitedStates) {
    ...
    for (unsigned int i19 = 0; i19 < keysNum_pj; i19++) {
...
void add_state_to_map(const state& s, const TS& ts, mt19* visitedStates) {
    ...
    (*visitedStates)[bs_pj][s.p[0]][s.p[1]][s.p[8]][s.c[8]][s.p[7]][s.c[7]][s.p[6]][s.c[6]][s.p[5]][s.c[5]][s.p[4]][s.c[4]][s.p[3]][s.c[3]][s.p[2]][s.c[2]] = true;
}
