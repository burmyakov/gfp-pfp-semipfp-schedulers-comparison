...
bool find_dominating_state_lpts(const state& s, const TS& ts, mt18& visitedStates) {
    ...
    if (s.p[1] <= ts.P[1] - ts.C[1]) { // s.c[1] == 0
        for (mt20::iterator itr20 = visitedStates.begin(); itr20 != visitedStates.end(); itr20++) {
            if (itr20->first > s.p[1]) break;
                ...
                if (find_dominating_state_lpts(s, ts, itr20->second)) return true;
            }
        }
    } else { // s.c[1] > 0
...
void add_state_to_map(const state& s, const TS& ts, mt23* visitedStates) {
    ...
    (*visitedStates)[bs_pj][s.p[0]][s.p[1]][s.p[10]][s.c[10]][s.p[9]][s.c[9]][s.p[8]][s.c[8]][s.p[7]][s.c[7]][s.p[6]][s.c[6]][s.p[5]][s.c[5]][s.p[4]][s.c[4]][s.p[3]][s.c[3]][s.p[2]][s.c[2]] = true;
}
...
bool test_11th_task(const bool verbose_, const unsigned short m_, const TS& ts_, unsigned long int& savedStatesNum_, unsigned long int& visitedStatesNum_) {
