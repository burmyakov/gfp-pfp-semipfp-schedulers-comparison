...
bool find_dominating_state_lpts(const state& s, const TS& ts, mt24& visitedStates) {
    
    for (mt24::iterator itr24 = visitedStates.begin(); itr24 != visitedStates.end(); itr24++) {
        if (itr24->first > s.p[13]) break;
        else {
            for (mt23::reverse_iterator itr23 = (itr24->second).rbegin(); itr23 != (itr24->second).rend(); itr23++) {
                if (itr23->first < s.c[13]) break;
                else {
                    for (mt22::iterator itr22 = (itr23->second).begin(); itr22 != (itr23->second).end(); itr22++) {
                        if (itr22->first > s.p[12]) break;
                        else {
                            for (mt21::reverse_iterator itr21 = (itr22->second).rbegin(); itr21 != (itr22->second).rend(); itr21++) {
                                if (itr21->first < s.c[12]) break;
                                else {
                                    for (mt20::iterator itr20 = (itr21->second).begin(); itr20 != (itr21->second).end(); itr20++) {
                                        if (itr20->first > s.p[11]) break;
                                        else {
                                            for (mt19::reverse_iterator itr19 = (itr20->second).rbegin(); itr19 != (itr20->second).rend(); itr19++) {
                                                if (itr19->first < s.c[11]) break;
                                                else {
                                                    for (mt18::iterator itr18 = (itr19->second).begin(); itr18 != (itr19->second).end(); itr18++) {
                                                        if (itr18->first > s.p[10]) break;
                                                        else {
                                                            for (mt17::reverse_iterator itr17 = (itr18->second).rbegin(); itr17 != (itr18->second).rend(); itr17++) {
                                                                if (itr17->first < s.c[10]) break;
                                                                else {
                                                                    for (mt16::iterator itr16 = (itr17->second).begin(); itr16 != (itr17->second).end(); itr16++) {
                                                                        if (itr16->first > s.p[9]) break;
                                                                        else {
                                                                            for (mt15::reverse_iterator itr15 = (itr16->second).rbegin(); itr15 != (itr16->second).rend(); itr15++) {
                                                                                if (itr15->first < s.c[9]) break;
                                                                                else {
...
bool find_dominating_state_hpt2(const state& s, const TS& ts, mt26& visitedStates) {
    
    if (s.p[1] <= ts.P[1] - ts.C[1]) { // s.c[1] == 0
        for (mt26::iterator itr26 = visitedStates.begin(); itr26 != visitedStates.end(); itr26++) {
            if (itr26->first > s.p[1]) break;
            else {
                if (find_dominating_state_lpts(s, ts, itr26->second)) return true;
            }
        }
    } else { // s.c[1] > 0
...
bool find_dominating_state_hpt1(const state& s, const TS& ts, mt28& visitedStates) {
    
    if (s.p[0] <= ts.P[0] - ts.C[0]) { // s.c[0] == 0
        for (mt28::iterator itr28 = visitedStates.begin(); itr28 != visitedStates.end(); itr28++) {
            if (itr28->first > s.p[0]) break;
            else {
                if (find_dominating_state_hpt2(s, ts, itr28->second)) return true;
            }
        }
    } else { // s.c[0] > 0
...
bool find_dominating_state(const state& s, const unsigned short m, const TS& ts, mt29* visitedStates) {
    
    unsigned int keysNum_pj = 0;
    get_keys_pj(s, m, keysNum_pj, bKeys_pj);
    
    for (unsigned int i29 = 0; i29 < keysNum_pj; i29++) {
        if ((*visitedStates).find((*bKeys_pj)[i29]) != (*visitedStates).end()) {
            if (find_dominating_state_hpt1(s, ts, (*visitedStates)[(*bKeys_pj)[i29]])) return true;
        }
    }
...
void add_state_to_map(const state& s, const TS& ts, mt29* visitedStates) {
    
    my_bitset bs_pj;
    for (int i = 0; i < ts.n; i++) if (s.c[i] > 0) bs_pj.set(i, 1); else bs_pj.set(i, 0);
    
    (*visitedStates)[bs_pj][s.p[0]][s.p[1]][s.p[13]][s.c[13]][s.p[12]][s.c[12]][s.p[11]][s.c[11]][s.p[10]][s.c[10]][s.p[9]][s.c[9]][s.p[8]][s.c[8]][s.p[7]][s.c[7]][s.p[6]][s.c[6]][s.p[5]][s.c[5]][s.p[4]][s.c[4]][s.p[3]][s.c[3]][s.p[2]][s.c[2]] = true;
}
...
bool update_map(const state& s, const unsigned short m, const TS& ts, mt29* visitedStates) {
...
void init(const unsigned short m, TS& ts) {
    
    bKeys_pj = new vector<my_bitset>(pow(2.0, ts.n));
    
    unsigned short* rtsGuan = new unsigned short[ts.n];
...
bool analyse_state(state& s, const TS& ts, const unsigned short m, priority_queue<state>& q, mt29* generated, const bool verbose) {
...
bool populate(const unsigned short m, const TS& ts_, const bool verbose) {
    ...
    if (verbose) {
        ...
        cout << "Visited states with tau_14 pending: " << visitedStatesNum_tauKpending << "  ( " << (float)((int)(visitedStatesNum_tauKpending*10000/visitedStatesNum))/100 << " %)" << endl << endl;
    }
    
    return schedulable;
}
} // end of namespace NS_14tasks





bool test_14th_task(const bool verbose_, const unsigned short m_, const TS& ts_, unsigned long int& savedStatesNum_, unsigned long int& visitedStatesNum_) {
    
    bool schedulable = NS_14tasks::populate(m_, ts_, verbose_);
    savedStatesNum_ = NS_14tasks::savedStatesNum;
    visitedStatesNum_ = NS_14tasks::visitedStatesNum;
