...
#include "../custom_types/map_10_tasks.h"
...
bool find_dominating_state_lpts(const state& s, const TS& ts, mt16& visitedStates) {
...
bool find_dominating_state_hpt2(const state& s, const TS& ts, mt18& visitedStates) {
    
    if (s.p[1] <= ts.P[1] - ts.C[1]) { // s.c[1] == 0
        for (mt18::iterator itr18 = visitedStates.begin(); itr18 != visitedStates.end(); itr18++) {
            if (itr18->first > s.p[1]) break;
            else {
                if (find_dominating_state_lpts(s, ts, itr18->second)) return true;
            }
        }
    } else { // s.c[1] > 0
...
bool find_dominating_state_hpt1(const state& s, const TS& ts, mt20& visitedStates) {
    
    if (s.p[0] <= ts.P[0] - ts.C[0]) { // s.c[0] == 0
        for (mt20::iterator itr20 = visitedStates.begin(); itr20 != visitedStates.end(); itr20++) {
            if (itr20->first > s.p[0]) break;
            else {
                if (find_dominating_state_hpt2(s, ts, itr20->second)) return true;
            }
        }
    } else { // s.c[0] > 0
...
bool find_dominating_state(const state& s, const unsigned short m, const TS& ts, mt21* visitedStates) {
    
    unsigned int keysNum_pj = 0;
    get_keys_pj(s, m, keysNum_pj, bKeys_pj);
    
    for (unsigned int i21 = 0; i21 < keysNum_pj; i21++) {
        if ((*visitedStates).find((*bKeys_pj)[i21]) != (*visitedStates).end()) {
            if (find_dominating_state_hpt1(s, ts, (*visitedStates)[(*bKeys_pj)[i21]])) return true;
        }
    }
...
void add_state_to_map(const state& s, const TS& ts, mt21* visitedStates) {
    
    my_bitset bs_pj;
    for (int i = 0; i < ts.n; i++) if (s.c[i] > 0) bs_pj.set(i, 1); else bs_pj.set(i, 0);
    
    (*visitedStates)[bs_pj][s.p[0]][s.p[1]][s.p[9]][s.c[9]][s.p[8]][s.c[8]][s.p[7]][s.c[7]][s.p[6]][s.c[6]][s.p[5]][s.c[5]][s.p[4]][s.c[4]][s.p[3]][s.c[3]][s.p[2]][s.c[2]] = true;
}
...
bool update_map(const state& s, const unsigned short m, const TS& ts, mt21* visitedStates) {
...
bool analyse_state(state& s, const TS& ts, const unsigned short m, priority_queue<state>& q, mt21* generated, const bool verbose) {
...
bool populate(const unsigned short m, const TS& ts_, const bool verbose) {
    ...
    if (verbose) {
        ...
        if (savedStatesNum > 0) cout << "Saved states with tau_10 pending: " << savedStatesNum_tauKpending << "  ( " << (float)((int)(savedStatesNum_tauKpending*10000/savedStatesNum))/100 << " %)" << endl;
        cout << "Visited states:\t\t" << visitedStatesNum << endl;
        cout << "Visited states with tau_10 pending: " << visitedStatesNum_tauKpending << "  ( " << (float)((int)(visitedStatesNum_tauKpending*10000/visitedStatesNum))/100 << " %)" << endl << endl;
    }
    
    return schedulable;
}
} // end of namespace NS_10tasks





bool test_10th_task(const bool verbose_, const unsigned short m_, const TS& ts_, unsigned long int& savedStatesNum_, unsigned long int& visitedStatesNum_) {
    
    bool schedulable = NS_10tasks::populate(m_, ts_, verbose_);
    savedStatesNum_ = NS_10tasks::savedStatesNum;
    visitedStatesNum_ = NS_10tasks::visitedStatesNum;
