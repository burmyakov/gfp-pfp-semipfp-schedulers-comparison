...
pair<string, string> generate_file_names() {
    ...
    string fileNameTasks = string("input_task_sets/task_sets_varying_n[nheavy_4_U_4_Dratio5_PDratio_1.5]_Dmin6.txt");
    string fileNameResults = string("results/results_varying_n[nheavy_4_U_4_Dratio5_PDratio_1.5]_Dmin6.txt");
...
tuple<bool, bool, bool, bool, bool, bool, bool, bool, bool, bool, unsigned int, unsigned int, long, long, long, long, long, long, long, long, long, long> test_schedulers(const unsigned short m, const unsigned short n, const unsigned short n_heavy, TS& ts, const bool verbose) {
    ...
    bool gfpDCMPO_sched = gfp_test_dcmpo(m, ts);
    //bool gfpDCMPO_sched = true;
    ...
    bool gfpDkC_sched = gfp_test_dkc(m, ts);
    ...
    start = std::chrono::system_clock::now();
    bool gfpDC_sched = gfp_test_dc(m, ts);
    end = std::chrono::system_clock::now();
    elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    t_DC = elapsed.count();
    ...
    start = std::chrono::system_clock::now();
    bool gfpDM_sched = gfp_test_dm(m, ts);
    end = std::chrono::system_clock::now();
    elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    t_DM = elapsed.count();
    ...
    start = std::chrono::system_clock::now();
    bool gfpDD_sched = gfp_test_dd(m, ts);
    end = std::chrono::system_clock::now();
    elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    t_DD = elapsed.count();
...
int main(){
    ...
    while (fileTaskSets.good()) {
        ...
        /*if (fabs(Dratio_prev - Dratio) >= 0.1) {
            caseNum++;
            Dratio_prev = Dratio;
        }*/
        ...
        // Sort tasks, having the same Di, by decreasing Ci
        /*for (unsigned short i = 1; i < ts.n; ++i)
            if ((ts.D[i-1] == ts.D[i]) && (ts.C[i-1] < ts.C[i]))
                swap(ts.C[i-1], ts.C[i]);*/
        ...
        /*gfpDM[n_heavy] += gfpDM_sched;
        gfpDCMPO[n_heavy] += gfpDCMPO_sched;
        gfpDkC[n_heavy] += gfpDkC_sched;
        gfpDC[n_heavy] += gfpDC_sched;
        gfpOPA[n_heavy] += gfpOpa_sched;
        pfpFFD[n_heavy] += pfpFFD_sched;
        pfpWFD[n_heavy] += pfpWFD_sched;
        pfpOPA[n_heavy] += pfpOpt_sched;*/
        
        
        /*if (gfpDM_sched > schedDM1) {
            cout << "gfpDM != prev" << endl;
            cout << "gfpDM_sched: " << gfpDM_sched << endl;
            cout << "schedDM1: " << schedDM1 << endl;
            abort();
        }*/
        //gfpDM_sched = gfpDM_sched_prev;
        
        /*if (gfpOpa_sched > schedGFPopt) {
            cout << "gfpOpa > prev" << endl;
            cout << "gfpOpa_sched: " << gfpOpa_sched << endl;
            cout << "schedGFPopt: " << schedGFPopt << endl;
            abort();
        }*/
        //gfpOpa_sched = gfpOpa_sched_prev;
        
        /*if (pfpFFD_sched != pfpFFD_sched_prev) {
            cout << "pfpFFD != prev" << endl;
            abort();
        }*/
        //pfpFFD_sched = pfpFFD_sched_prev;
        
        /*if (pfpWFD_sched != pfpWFD_sched_prev) {
            cout << "pfpWFD != prev" << endl;
            abort();
        }*/
        //pfpWFD_sched = pfpWFD_sched_prev;
        
        /*if (pfpFFD_sched != schedPFPopt) {
            cout << "pfpOpt != prev" << endl;
            abort();
        }*/
        //pfpFFD_sched = pfpFFD_sched_prev;
        
        /*if ((!gfpOpa_sched) && ((gfpDCMPO_sched || gfpDkC_sched) || gfpDC_sched)) {
            cout << "ERROR in GFP code !!!!" << endl;
            abort();
        }*/
    ...
    /*cout << "Resulted statistics:" << endl;
    for (int i = 0; i <= casesCounter; ++i) {
        cout << endl << endl << "case no: " << i << endl << endl;
        
        cout << "GFP OPA sched: "   << gfpOPASchedCasesNum[i]  << endl;
        cout << "PFP OPT sched: "   << pfpOPTSchedCasesNum[i]  << endl << endl;
        
        cout << "GFP DM sched: "    << gfpDMSchedCasesNum[i]   << endl;
        cout << "GFP DCMPO sched: " << gfpDCMPOSchedCasesNum[i] << endl;
        cout << "GFP DkC sched: "   << gfpDkCSchedCasesNum[i]  << endl;
        cout << "GFP DC sched: "    << gfpDCSchedCasesNum[i]   << endl;
        
        cout << "PFP FFD sched: "   << pfpFFDSchedCasesNum[i]  << endl;
        cout << "PFP WFD sched: "   << pfpWFDSchedCasesNum[i]  << endl;
